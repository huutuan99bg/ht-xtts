...
def normalize_vietnamese_text(text: str) -> str:
    # Bỏ dấu câu dư thừa và khoảng trắng
    ...
    # Chuẩn hoá một số từ phổ biến cho TTS
    text = text.replace("AI", "Ây Ai")
    text = text.replace("A.I", "Ây Ai")
...
def run_tts(lang, tts_text, speaker_audio_file, use_deepfilter, normalize_text, top_k = 30, top_p = 0.85, temperature = 0.7, speed = 1):
    ...
    print(f"Generated audio time {round(time() - ts, 2)}s")
    return f"Speech generated! {round(time() - ts, 2)}s", out_path
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column() as col2:
                ...
                top_k = gr.Slider(1, 1000, 30, step=1, label="Top K")
                top_p = gr.Slider(0.0, 1.0, 0.85, step=0.05, label="Top P")
                temperature = gr.Slider(0.0, 1.0, 0.7, step=0.05, label="Temperature")
                speed = gr.Slider(0.0, 3.0, 1, step=0.05, label="speed")
